...
repo's life: continuously, only at the beginning, only at the end, or one-off.
  ...
  >>> THIS SCRIPT COUNTS ONLY ROWS WHERE qualityGateStatus == "OK". <<<
  (For the unfiltered version that counts everything, see sonar_cadence_all.py.)
...
def parse_args():
    ...
    p.add_argument("--out", default="sonar_cadence_ok_out")
